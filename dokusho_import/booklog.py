...
列仕様（公式ヘルプ「他の読書管理サイトからブクログへデータを移行したい」
「インポート機能」「サービスIDとアイテムIDとは何ですか」より）:

    サービスID, アイテムID, 13桁ISBN, カテゴリ, 評価, 読書状況,
    レビュー, タグ, 非公開メモ, 登録日時, 読了日

確定している制約:
  - 「アイテムID」「13桁ISBN」はどちらか必須
  - 登録日時は "yyyy-mm-dd hh:mm:ss"。これを守らないと全件が
    インポート日時に上書きされる
  - サービスID: Amazon.co.jp=1, Amazon.com=2, iTunes=4

UNCONFIRMED にまとめた項目は公式ページ本文が読めていないため未確定。
ここだけ差し替えれば済むように1か所に隔離してある。
"""
...
実データ1件でインポートを試し、上の3点を確定させてから全件を流すこと。
"""
